...
public class Proxy
{
    ...
    public static boolean proxy(final SocketBase frontend_,
                                final SocketBase backend_,
                                final SocketBase capture_)
    {
        ...
        long more;
        Msg msg;
        final PollItem items[] = new PollItem[2];
            ...
            while (!Thread.currentThread().isInterrupted()) {
                ...
                if (items[0].isReadable()) {
                    while (true) {
                        ...
                        if (capture_ != null) {
                            final Msg ctrl = new Msg(msg);
                            success = capture_.send(ctrl,
                                                    more > 0 ? ZMQ.ZMQ_SNDMORE
                                                            : 0);
                            if (!success) {
                                return false;
                            }
                        ...
                        success = backend_.send(msg, more > 0 ? ZMQ.ZMQ_SNDMORE
                                                             : 0);
                        if (!success) {
                            return false;
                        }
                ...
                if (items[1].isReadable()) {
                    while (true) {
                        ...
                        success = frontend_.send(msg,
                                                 more > 0 ? ZMQ.ZMQ_SNDMORE : 0);
                        if (!success) {
                            return false;
                        }
                        if (more == 0) {
