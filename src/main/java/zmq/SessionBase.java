                                    ...
                                    IMsgSink, IMsgSource
{
    ...
    private final IOThread io_thread;
    ...
    public static SessionBase create(final IOThread io_thread_,
                                     final boolean connect_,
                                     final SocketBase socket_,
                                     final Options options_, final Address addr_)
    {
        ...
        case ZMQ.ZMQ_REQ:
            s = new Req.ReqSession(io_thread_, connect_, socket_, options_,
                                   addr_);
            break;
        case ZMQ.ZMQ_DEALER:
            s = new Dealer.DealerSession(io_thread_, connect_, socket_,
                                         options_, addr_);
            break;
        case ZMQ.ZMQ_REP:
            s = new Rep.RepSession(io_thread_, connect_, socket_, options_,
                                   addr_);
            break;
        case ZMQ.ZMQ_ROUTER:
            s = new Router.RouterSession(io_thread_, connect_, socket_,
                                         options_, addr_);
            break;
        case ZMQ.ZMQ_PUB:
            s = new Pub.PubSession(io_thread_, connect_, socket_, options_,
                                   addr_);
            break;
        case ZMQ.ZMQ_XPUB:
            s = new XPub.XPubSession(io_thread_, connect_, socket_, options_,
                                     addr_);
            break;
        case ZMQ.ZMQ_SUB:
            s = new Sub.SubSession(io_thread_, connect_, socket_, options_,
                                   addr_);
            break;
        case ZMQ.ZMQ_XSUB:
            s = new XSub.XSubSession(io_thread_, connect_, socket_, options_,
                                     addr_);
            break;

        case ZMQ.ZMQ_PUSH:
            s = new Push.PushSession(io_thread_, connect_, socket_, options_,
                                     addr_);
            break;
        case ZMQ.ZMQ_PULL:
            s = new Pull.PullSession(io_thread_, connect_, socket_, options_,
                                     addr_);
            break;
        case ZMQ.ZMQ_PAIR:
            s = new Pair.PairSession(io_thread_, connect_, socket_, options_,
                                     addr_);
            break;
        default:
    ...
    public SessionBase(final IOThread io_thread_, final boolean connect_,
    ...
    public void attach_pipe(final Pipe pipe_)
    {
    ...
    public Msg pull_msg()
    {
        ...
        if (!identity_sent) {
            final Msg msg_ = new Msg(options.identity_size);
    ...
    @Override
    public int push_msg(final Msg msg_)
    {
    ...
    @Override
    protected void process_attach(final IEngine engine_)
    {
        assert (engine_ != null);

        // Create the pipe if it does not exist yet.
        if (pipe == null && !is_terminating()) {
            final ZObject[] parents = { this, socket };
            final Pipe[] pipes = { null, null };
            final int[] hwms = { options.rcvhwm, options.sndhwm };
            final boolean[] delays = { options.delay_on_close,
    ...
    protected void process_term(final int linger_)
    {
        ...
        // Start pipe termination process. Delay the termination till all
        // messages
    ...
    public void timer_event(final int id_)
    {
        ...
        // Ask pipe to terminate even though there may be pending messages in
        // it.
        assert (pipe != null);
    ...
    private void detached()
    {
        ...
        if (pipe != null
            && (options.type == ZMQ.ZMQ_SUB || options.type == ZMQ.ZMQ_XSUB)) {
            pipe.hiccup();
        }

    }

    private void start_connecting(final boolean wait_)
    {
        assert (connect);
        ...
        if (addr.protocol().equals("tcp")) {
            final TcpConnecter connecter = new TcpConnecter(io_thread, this,
                                                            options, addr,
                                                            wait_);
        ...
        if (addr.protocol().equals("ipc")) {
            final IpcConnecter connecter = new IpcConnecter(io_thread, this,
                                                            options, addr,
                                                            wait_);
            // alloc_assert (connecter);
            launch_child(connecter);
