...
abstract public class EncoderBase implements IEncoder
{
    ...
    protected EncoderBase(final int bufsize_)
    {
        buffersize = bufsize_;
    ...
    public Transfer get_data(ByteBuffer buffer)
    {
        ...
        while (buffer.hasRemaining()) {
            ...
            if (write_channel != null) {
                buffer.flip();
                final Transfer t = new Transfer.FileChannelTransfer(
                                                                    buffer,
                                                                    write_channel,
                                                                    write_pos,
                                                                    to_write);
            ...
            if (buf.position() == 0 && to_write >= buffersize) {
                Transfer t;
                final ByteBuffer b = ByteBuffer.wrap(write_buf);
            ...
            final int to_copy = Math.min(to_write, buffer.remaining());
            if (to_copy > 0) {
    ...
    protected void state(final int state_)
    {
        next = state_;
    }
    ...
    protected void next_step(final Msg msg_, final int state_,
                             final boolean beginning_)
    {
        if (msg_ == null) {
    ...
    protected void next_step(final byte[] buf_, final int to_write_,
                             final int next_, final boolean beginning_)
    {
        write_buf = buf_;
        write_channel = null;
        write_pos = 0;
    ...
    protected void next_step(final FileChannel ch, final long pos_,
                             final long to_write_, final int next_,
                             final boolean beginning_)
    {
        write_buf = null;
        write_channel = ch;
