...
public class Rep extends Router
{
    ...
    {
        public RepSession(final IOThread io_thread_, final boolean connect_,
    ...
    public Rep(final Ctx parent_, final int tid_, final int sid_)
    {
    ...
    protected Msg xrecv()
    {
        // If we are in middle of sending a reply, we cannot receive next
        // request.
        if (sending_reply) {
        ...
        if (request_begins) {
            while (true) {
                ...
                if (msg_.hasMore()) {
                    ...
                    final boolean bottom = (msg_.size() == 0);
