...
public class Mtrie
{
    ...
    public boolean add(final byte[] prefix_, final Pipe pipe_)
    {
    ...
    public boolean add(final byte[] prefix_, final int start_, final Pipe pipe_)
    {
        return add_helper(prefix_, start_, pipe_);
    }

    private boolean add_helper(final byte[] prefix_, final int start_,
                               final Pipe pipe_)
    {
        // We are at the node corresponding to the prefix. We are done.
        if (prefix_ == null || prefix_.length == start_) {
            final boolean result = pipes == null;
            if (pipes == null) {
    ...
    private Mtrie[] realloc(final Mtrie[] table, final int size,
                            final boolean ended)
    {
    ...
    public boolean rm(final Pipe pipe_, final IMtrieHandler func,
                      final Object arg)
    {
        return rm_helper(pipe_, new byte[0], 0, 0, func, arg);
    }

    private boolean rm_helper(final Pipe pipe_, byte[] buff_, int buffsize_,
                              int maxbuffsize_, final IMtrieHandler func_,
                              final Object arg_)
    {
        ...
        if (count == 1) {
            ...
            buffsize_++;
            next[0].rm_helper(pipe_, buff_, buffsize_, maxbuffsize_, func_,
                              arg_);
        ...
        for (int c = 0; c != count; c++) {
            ...
            if (next[c] != null) {
                next[c].rm_helper(pipe_, buff_, buffsize_ + 1, maxbuffsize_,
                                  func_, arg_);
                ...
                if (next[c].is_redundant()) {
                    ...
                    // minimum index. Conversely, the last non-redundant,
                    // non-null
        ...
        // Compact the node table if possible
            ...
            final Mtrie node = next[new_min - min];
            assert (node != null);
            next = null;
            next = new Mtrie[] { node };
            count = 1;
    ...
    public boolean rm(final byte[] prefix_, final int start_, final Pipe pipe_)
    {
        return rm_helper(prefix_, start_, pipe_);
    }

    private boolean rm_helper(final byte[] prefix_, final int start_,
                              final Pipe pipe_)
    {
        if (prefix_ == null || prefix_.length == start_) {
            if (pipes != null) {
                final boolean erased = pipes.remove(pipe_);
                assert (erased);
                if (pipes.isEmpty()) {
                    pipes = null;
                }
        ...
        final Mtrie next_node = count == 1 ? next[0] : next[c - min];

        if (next_node == null) {
            return false;
        }

        final boolean ret = next_node.rm_helper(prefix_, start_ + 1, pipe_);
    ...
    public void match(final byte[] data_, int size_, final IMtrieHandler func_,
                      final Object arg_)
    {
        Mtrie current = this;
        ...
        while (true) {
            ...
            if (current.pipes != null) {
                for (final Pipe it : current.pipes) {
                    func_.invoke(it, null, 0, arg_);
                }
            }

            // If we are at the end of the message, there's nothing more to
            // match.
            if (size_ == 0) {
                break;
            }
