...
public class PollItem
{
    ...
    public PollItem(final SocketBase s_)
    {
        s = s_;
        c = null;
    ...
    public PollItem(final SocketBase s_, final int ops)
    {
        s = s_;
        c = null;
        init(ops);
    ...
    public PollItem(final SelectableChannel c_, final int ops)
    {
        s = null;
        c = c_;
        init(ops);
    ...
    private void init(final int ops)
    {
        zinterest = ops;
        int interest_ = 0;
    ...
    public final int interestOps(final int ops)
    {
        init(ops);
        return interest;
    }

    public final int readyOps(final SelectionKey key, final int nevents)
    {
        ready = 0;

        if (s != null) {
            final int events = s.getsockopt(ZMQ.ZMQ_EVENTS);
            if (events < 0) {
                return -1;
            }

            if ((zinterest & ZMQ.ZMQ_POLLOUT) > 0
                && (events & ZMQ.ZMQ_POLLOUT) > 0) {
                ready |= ZMQ.ZMQ_POLLOUT;
            }
            if ((zinterest & ZMQ.ZMQ_POLLIN) > 0
                && (events & ZMQ.ZMQ_POLLIN) > 0) {
