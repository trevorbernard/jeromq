...
public class Pipe extends ZObject
{
    ...
    enum State
    {
        ACTIVE, DELIMITED, PENDING, TERMINATING, TERMINATED, DOUBLE_TERMINATED
    }
    ...
    // pipepair function.
    private Pipe(final ZObject parent_, final YPipe<Msg> inpipe_,
                 final YPipe<Msg> outpipe_, final int inhwm_,
                 final int outhwm_, final boolean delay_)
    {
        super(parent_);
        inpipe = inpipe_;
        outpipe = outpipe_;
    ...
    public static void pipepair(final ZObject[] parents_, final Pipe[] pipes_,
                                final int[] hwms_, final boolean[] delays_)
    {

        // Creates two pipe objects. These objects are connected by two ypipes,
        // each to pass messages in one direction.

        final YPipe<Msg> upipe1 = new YPipe<Msg>(
                                                 Config.MESSAGE_PIPE_GRANULARITY.getValue());
        final YPipe<Msg> upipe2 = new YPipe<Msg>(
                                                 Config.MESSAGE_PIPE_GRANULARITY.getValue());

        pipes_[0] = new Pipe(parents_[0], upipe1, upipe2, hwms_[1], hwms_[0],
                             delays_[0]);
        pipes_[1] = new Pipe(parents_[1], upipe2, upipe1, hwms_[0], hwms_[1],
                             delays_[1]);
    ...
    private void set_peer(final Pipe peer_)
    {
    ...
    public void set_event_sink(final IPipeEvents sink_)
    {
        assert (sink == null);
        sink = sink_;
    }
    ...
    public void set_identity(final Blob identity_)
    {
        identity = identity_;
    }
    ...
    public boolean check_write()
    {
        ...
        final boolean full = hwm > 0 && msgs_written - peers_msgs_read == (hwm);
    ...
    public boolean write(final Msg msg_)
    {
        if (!check_write()) {
            return false;
        }

        final boolean more = msg_.hasMore();
    ...
    @Override
    protected void process_hiccup(final Object pipe_)
    {
    ...
    protected void process_pipe_term()
    {
        ...
        // Otherwise we'll hang up in pending state till all the pending
        // messages
        // are sent.
        if (state == State.ACTIVE) {
            if (!delay) {
    ...
    {
        // Notify the user that all the references to the pipe should be
        // dropped.
        assert (sink != null);
    ...
    public void terminate(final boolean delay_)
    {
        // Overload the value specified at pipe creation.
        delay = delay_;

        // If terminate was already called, we can ignore the duplicit
        // invocation.
        ...
        if (outpipe != null) {
            ...
            // checked; thus the delimiter can be written even when the pipe is
            // full.
    ...
    private static boolean is_delimiter(final Msg msg_)
    {
        return msg_.isDelimiter();
    }

    // Computes appropriate low watermark from the given high watermark.
    private static int compute_lwm(final int hwm_)
    {
        // Compute the low water mark. Following point should be taken
        // into consideration:
        //
        // 1. LWM has to be less than HWM.
        // 2. LWM cannot be set to very low value (such as zero) as after
        // filling
        ...
        final int result = (hwm_ > Config.MAX_WM_DELTA.getValue() * 2) ? hwm_
                                                                         - Config.MAX_WM_DELTA.getValue()
                                                                      : (hwm_ + 1) / 2;
