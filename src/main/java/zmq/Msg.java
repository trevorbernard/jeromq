...
public class Msg
{
    ...
    public Msg(final int capacity)
    {
        ...
        this.flags = 0;
        this.size = capacity;
        this.buf = ByteBuffer.wrap(new byte[capacity])
    ...
    public Msg(final ByteBuffer src)
    {
        ...
        if (src.position() > 0) {
            throw new IllegalArgumentException(
                                               "ByteBuffer position is not zero, did you forget to flip it?");
        }
        this.type = TYPE_LMSG;
        this.flags = 0;
    ...
    public void setFlags(final int flags_)
    {
        flags |= flags_;
    }
    ...
    public void resetFlags(final int f)
    {
        flags = flags & ~f;
    }
    ...
    public Msg put(final byte b)
    {
        buf.put(b);
        return this;
    }

    public Msg put(final int index, final byte b)
    {
        buf.put(index, b);
        return this;
    }

    public Msg put(final byte[] src)
    {
        return put(src, 0, src.length);
    }

    public Msg put(final byte[] src, final int off, final int len)
    {
        if (src == null) {
            return this;
        }
    ...
    public Msg put(final ByteBuffer src)
    {
        buf.put(src);
        return this;
    }

    public int getBytes(final int index, final byte[] dst, final int off,
                        final int len)
    {
        final int count = Math.min(len, size);
        if (buf.isDirect()) {
    ...
    public int getBytes(final int index, final ByteBuffer bb, final int len)
    {
    ...
    {
        return String.format("#zmq.Msg{type=%s, size=%s, flags=%s}", type,
                             size, flags);
    }
}
