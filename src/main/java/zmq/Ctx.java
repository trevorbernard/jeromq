...
public class Ctx
{
    ...
    public static class Endpoint
    {
        ...
        public Endpoint(final SocketBase socket_, final Options options_)
        {
            socket = socket_;
            options = options_;
        }
    ...
    protected void destroy()
    {
        ...
        for (final IOThread it : io_threads) {
            it.stop();
        }
        for (final IOThread it : io_threads) {
            it.destroy();
    ...
    public void terminate()
    {
        ...
        if (!starting.get()) {
            slot_sync.lock();
            try {
                // Check whether termination was already underway, but
                // interrupted and now
                // restarted.
                final boolean restarted = terminating;
                terminating = true;
                ...
                if (!restarted) {
                    ...
                    // reaper
                    // thread to stop.
                    for (final SocketBase socket : sockets) {
                        socket.stop();
                    }
                    if (sockets.isEmpty()) {
                        reaper.stop();
            ...
            final Command cmd = term_mailbox.recv(-1);
            if (cmd == null) {
    ...
    public boolean set(final int option_, final int optval_)
    {
        ...
        else if (option_ == ZMQ.ZMQ_IO_THREADS && optval_ >= 0) {
            opt_sync.lock();
            try {
    ...
    public int get(final int option_)
    {
        int rc = 0;
        if (option_ == ZMQ.ZMQ_MAX_SOCKETS) {
            rc = max_sockets;
        }
        else if (option_ == ZMQ.ZMQ_IO_THREADS) {
            rc = io_thread_count;
        }
        else {
    ...
    public SocketBase create_socket(final int type_)
    {
        SocketBase s = null;
        slot_sync.lock();
        try {
            if (starting.compareAndSet(true, false)) {
                // Initialise the array of mailboxes. Additional three slots are
                // for
                ...
                for (int i = 2; i != ios + 2; i++) {
                    final IOThread io_thread = new IOThread(this, i);
                ...
                // In the unused part of the slot array, create a list of empty
                // slots.
                for (int i = slot_count - 1; i >= ios + 2; i--) {
                    empty_slots.add(i);
                    slots[i] = null;
                }
            ...
            final int slot = empty_slots.pollLast();

            // Generate new unique socket ID.
            final int sid = max_socket_id.incrementAndGet();
    ...
    public void destroy_socket(final SocketBase socket_)
    {
        slot_sync.lock();

        // Free the associated thread slot.
        try {
            final int tid = socket_.get_tid();
            empty_slots.add(tid);
            slots[tid].close();
            slots[tid] = null;
    ...
    public void send_command(final int tid_, final Command command_)
    {
    ...
    public IOThread choose_io_thread(final long affinity_)
    {
        ...
        for (int i = 0; i != io_threads.size(); i++) {
            if (affinity_ == 0 || (affinity_ & (1L << i)) > 0) {
                final int load = io_threads.get(i).get_load();
    ...
    public boolean register_endpoint(final String addr_,
                                     final Endpoint endpoint_)
    {
        endpoints_sync.lock();
    ...
    public void unregister_endpoints(final SocketBase socket_)
    {

        endpoints_sync.lock();

        try {
            final Iterator<Entry<String, Endpoint>> it = endpoints.entrySet()
                                                                  .iterator();
            while (it.hasNext()) {
                final Entry<String, Endpoint> e = it.next();
    ...
    public Endpoint find_endpoint(final String addr_)
    {
        ...
        try {
            ...
            // Increment the command sequence number of the peer so that it
            // won't
