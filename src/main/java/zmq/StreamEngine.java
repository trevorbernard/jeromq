...
public class StreamEngine implements IEngine, IPollEvents, IMsgSink
{
    ...
    public StreamEngine(final SocketChannel fd_, final Options options_,
                        final String endpoint_)
    {
        handle = fd_;
        inbuf = null;
        insize = 0;
    ...
    private DecoderBase new_decoder(final int size, final long max,
                                    final SessionBase session, final int version)
    {
    ...
    private EncoderBase new_encoder(final int size, final SessionBase session,
                                    final int version)
    {
    ...
    @Override
    public void plug(final IOThread io_thread_, final SessionBase session_)
    {
        assert (!plugged);
        plugged = true;
        ...
        try {
            custom = options.encoder != null
                     && options.encoder.getDeclaredField("RAW_ENCODER") != null;
        }
        catch (final SecurityException e) {
        }
    ...
    public void in_event()
    {
        ...
        final int processed = decoder.process_buffer(inbuf, insize);
    ...
    public void out_event()
    {
        ...
        // when we use custom encoder, we might want to close after sending a
        // response
        if (outsize == 0) {
    ...
    public void activate_out()
    {
        ...
        // Consequently, the latency should be better in request/reply
        // scenarios.
        out_event();
    }
    ...
    private boolean handshake()
    {
        ...
        while (greeting.position() < GREETING_SIZE) {
            ...
            if (greeting_output_buffer.limit() < GREETING_SIZE) {
                ...
                final int pos = greeting_output_buffer.position();
        ...
        if ((greeting.get(0) & 0xff) != 0xff || (greeting.get(9) & 0x01) == 0) {
            ...
            decoder = new_decoder(Config.IN_BATCH_SIZE.getValue(),
                                  options.maxmsgsize, null, 0);
            ...
            final ByteBuffer tmp = ByteBuffer.allocate(header_size);
        ...
        else if (greeting.get(version_pos) == 0) {
            // ZMTP/1.0 framing.
            encoder = new_encoder(Config.OUT_BATCH_SIZE.getValue(), null, 0);
            encoder.set_msg_source(session);
            ...
            // v1 framing protocol.
            encoder = new_encoder(Config.OUT_BATCH_SIZE.getValue(), session,
                                  V1Protocol.VERSION);

            decoder = new_decoder(Config.IN_BATCH_SIZE.getValue(),
                                  options.maxmsgsize, session,
                                  V1Protocol.VERSION);
        }
    ...
    private int write(final Transfer buf)
    {
        int nbytes = 0;
        try {
    ...
    private int read(final ByteBuffer buf)
    {
        int nbytes = 0;
        try {
