...
public class TcpConnecter extends Own implements IPollEvents
{
    ...
    private final boolean delayed_start;
    ...
    public TcpConnecter(final IOThread io_thread_, final SessionBase session_,
                        final Options options_, final Address addr_,
                        final boolean delayed_start_)
    {
    ...
    @Override
    public void process_term(final int linger_)
    {
        if (timer_started) {
    ...
    {
        final int rc_ivl = get_new_reconnect_ivl();
    ...
    private int get_new_reconnect_ivl()
    {
        ...
        final int this_interval = current_reconnect_ivl
    ...
    private boolean open() throws IOException
    {
        ...
        final Address.IZAddress resolved = addr.resolved();
        if (resolved == null) {
            throw new IOException("Address not resolved");
        }
        final SocketAddress sa = resolved.address();
        if (sa == null) {
        ...
        try {
            ...
            // this will happen if sa is bad. Address validation is not
            // documented but
    ...
    {
        final boolean finished = handle.finishConnect();
        assert finished;
        final SocketChannel ret = handle;
