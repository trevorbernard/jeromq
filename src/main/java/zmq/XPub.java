...
public class XPub extends SocketBase
{
    ...
    public static class XPubSession extends SessionBase
    {
        ...
        public XPubSession(final IOThread io_thread_, final boolean connect_,
    ...
    static {
        mark_as_matching = new Mtrie.IMtrieHandler()
        {
            ...
            @Override
            public void invoke(final Pipe pipe_, final byte[] data,
                               final int size, final Object arg_)
            {
                final XPub self = (XPub) arg_;
        ...
        send_unsubscription = new Mtrie.IMtrieHandler()
        {
            ...
            @Override
            public void invoke(final Pipe pipe_, final byte[] data_,
                               final int size, final Object arg_)
            {
                final XPub self = (XPub) arg_;
                ...
                if (self.options.type != ZMQ.ZMQ_PUB) {
                    ...
                    final byte[] unsub = new byte[size + 1];
                    unsub[0] = 0;
    ...
    public XPub(final Ctx parent_, final int tid_, final int sid_)
    {
    ...
    protected void xread_activated(final Pipe pipe_)
    {
        ...
        while ((sub = pipe_.read()) != null) {
            ...
            if (size > 0 && (data[0] == 0 || data[0] == 1)) {
                ...
                // can be
                // passed to used on next recv call. (Unsubscribe is not
                // verbose.)
                if (options.type == ZMQ.ZMQ_XPUB
                    && (unique || (data[0] == 1 && verbose))) {
    ...
    {
        final boolean msg_more = msg_.hasMore();

        // For the first part of multi-part message, find the matching pipes.
        if (!more) {
            subscriptions.match(msg_.data(), msg_.size(), mark_as_matching,
                                this);
        }

        // Send the message to all the pipes that were marked as matching
        // in the previous step.
        final boolean rc = dist.send_to_matching(msg_);
        if (!rc) {
            return false;
        }
    ...
    protected Msg xrecv()
    {
        ...
        final Blob first = pending_data.pollFirst();
        final Msg msg = new Msg(first.data());
        final int flags = pending_flags.pollFirst();
        msg.setFlags(flags);
        return msg;
    }
