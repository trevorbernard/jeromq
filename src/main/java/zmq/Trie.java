...
public class Trie
{
    ...
    public boolean add(final byte[] prefix_)
    {
        return add(prefix_, 0);
    }

    public boolean add(final byte[] prefix_, final int start_)
    {
    ...
    private Trie[] realloc(final Trie[] table, final int size,
                           final boolean ended)
    {
    ...
    public boolean rm(final byte[] prefix_, final int start_)
    {
        if (prefix_ == null || prefix_.length == start_) {
            if (refcnt == 0) {
                return false;
        ...
        final Trie next_node = count == 1 ? next[0] : next[c - min];

        if (next_node == null) {
            return false;
        }

        final boolean ret = next_node.rm(prefix_, start_ + 1);
    ...
    public boolean check(final byte[] data_)
    {
        ...
        while (true) {
            ...
            // We've checked all the data and haven't found matching
            // subscription.
    ...
    public void apply(final ITrieHandler func, final Object arg_)
    {
        apply_helper(null, 0, 0, func, arg_);
    }

    private void apply_helper(byte[] buff_, int buffsize_, int maxbuffsize_,
                              final ITrieHandler func_, final Object arg_)
    {
        ...
        for (int c = 0; c != count; c++) {
            ...
            if (next[c] != null) {
                next[c].apply_helper(buff_, buffsize_ + 1, maxbuffsize_, func_,
                                     arg_);
            }
        }
    }
