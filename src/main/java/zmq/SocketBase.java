...
public abstract class SocketBase extends Own implements IPollEvents,
                                            Pipe.IPipeEvents
{
    ...
    protected SocketBase(final Ctx parent_, final int tid_, final int sid_)
    {
        super(parent_, tid_);
        tag = 0xbaddecaf;
    ...
    public static SocketBase create(final int type_, final Ctx parent_,
                                    final int tid_, final int sid_)
    {
        SocketBase s = null;
        switch (type_) {
    ...
    private void check_protocol(final String protocol_)
    {
        // First check out whether the protcol is something we are aware of.
        if (!protocol_.equals("inproc") && !protocol_.equals("ipc")
            && !protocol_.equals("tcp") /*
                                         * && !protocol_.equals("pgm") &&
                                         * !protocol_.equals("epgm")
                                         */) {
        ...
        if ((protocol_.equals("pgm") || protocol_.equals("epgm"))
            ...
            throw new UnsupportedOperationException(protocol_ + ",type="
                                                    + options.type);
        }
    ...
    private void attach_pipe(final Pipe pipe_)
    {
        attach_pipe(pipe_, false);
    }

    private void attach_pipe(final Pipe pipe_, final boolean icanhasall_)
    {
    ...
    public void setsockopt(final int option_, final Object optval_)
    {
    ...
    public int getsockopt(final int option_)
    {
        ...
        if (option_ == ZMQ.ZMQ_EVENTS) {
            final boolean rc = process_commands(0, false);
    ...
    public Object getsockoptx(final int option_)
    {
        if (ctx_terminated) {
            throw new ZError.CtxTerminatedException();
        }
    ...
    public boolean bind(final String addr)
    {
        ...
        final boolean brc = process_commands(0, false);
        if (!brc) {
            return false;
        }
        ...
        if (protocol.equals("inproc")) {
            final Ctx.Endpoint endpoint = new Ctx.Endpoint(this, options);
            final boolean rc = register_endpoint(addr, endpoint);
            if (rc) {
        ...
        if (protocol.equals("tcp")) {
            final TcpListener listener = new TcpListener(io_thread, this,
                                                         options);
            final int rc = listener.set_address(address);
            if (rc != 0) {
                listener.destroy();
        ...
        if (protocol.equals("ipc")) {
            final IpcListener listener = new IpcListener(io_thread, this,
                                                         options);
            final int rc = listener.set_address(address);
            if (rc != 0) {
                listener.destroy();
    ...
    public boolean connect(final String addr_)
    {
        if (ctx_terminated) {
            throw new ZError.CtxTerminatedException();
        }
        ...
        if (protocol.equals("inproc")) {
            ...
            final Ctx.Endpoint peer = find_endpoint(addr_);
            if (peer.socket == null) {
                return false;
            }
            ...
            final ZObject[] parents = { this, peer.socket };
            final Pipe[] pipes = { null, null };
            final int[] hwms = { sndhwm, rcvhwm };
            final boolean[] delays = { options.delay_on_disconnect,
            ...
            if (peer.options.recv_identity) {
                final Msg id = new Msg(options.identity_size);
                id.put(options.identity, 0, options.identity_size);
                id.setFlags(Msg.IDENTITY);
                final boolean written = pipes[0].write(id);
                assert (written);
                pipes[0].flush();
            }
            ...
            if (options.recv_identity) {
                final Msg id = new Msg(peer.options.identity_size);
                id.put(peer.options.identity, 0, peer.options.identity_size);
                id.setFlags(Msg.IDENTITY);
                final boolean written = pipes[1].write(id);
                assert (written);
                pipes[1].flush();
            }
            ...
            // peer's
            // seqnum was incremented in find_endpoint function. We don't need
            // it
            // increased here.
        ...
        final boolean ipv4only = options.ipv4only != 0 ? true : false;
        final Address paddr = new Address(protocol, address, ipv4only);

        // Resolve address (if needed by the protocol)
        paddr.resolve();

        // Create session.
        final SessionBase session = SessionBase.create(io_thread, true, this,
                                                       options, paddr);
        assert (session != null);
        ...
        if (options.delay_attach_on_connect != 1 || icanhasall) {
            ...
            final ZObject[] parents = { this, session };
            final Pipe[] pipes = { null, null };
            final int[] hwms = { options.sndhwm, options.rcvhwm };
            final boolean[] delays = { options.delay_on_disconnect,
                                      options.delay_on_close };
            Pipe.pipepair(parents, pipes, hwms, delays);
    ...
    private void add_endpoint(final String addr_, final Own endpoint_)
    {
    ...
    public boolean term_endpoint(final String addr_)
    {
        ...
        // Process pending commands, if any, since there could be pending
        // unprocessed process_own()'s
        ...
        if (protocol.equals("inproc")) {
            ...
            final Iterator<Entry<String, Pipe>> it = inprocs.entrySet()
                                                            .iterator();
            while (it.hasNext()) {
        ...
        final Iterator<Entry<String, Own>> it = endpoints.entrySet().iterator();

        while (it.hasNext()) {
            final Entry<String, Own> e = it.next();
    ...
    public boolean send(final Msg msg_, final int flags_)
    {
        if (ctx_terminated) {
        ...
        // Process pending commands, if any.
        final boolean brc = process_commands(0, true);
        if (!brc) {
            return false;
        }
        ...
        final long end = timeout < 0 ? 0 : (Clock.now_ms() + timeout);
    ...
    public Msg recv(final int flags_)
    {
        ...
        // described above) from the one used by 'send'. This is because
        // counting
        ...
        // If the message cannot be fetched immediately, there are two
        // scenarios.
    ...
    // its poller.
    public void start_reaping(final Poller poller_)
    {
    ...
    private boolean process_commands(final int timeout_, final boolean throttle_)
    {
        Command cmd;
        ...
        if (timeout_ != 0) {
            ...
            final long tsc = 0; // save cpu Clock.rdtsc ();
            ...
            if (tsc != 0 && throttle_) {
                ...
                if (tsc >= last_tsc
                    && tsc - last_tsc <= Config.MAX_COMMAND_DELAY.getValue()) {
                    return true;
                }
                last_tsc = tsc;
            }
        ...
        if (ctx_terminated) {
            errno.set(ZError.ETERM); // Do not raise exception at the blocked
                                     // operation
            return false;
        }
    ...
    protected void process_stop()
    {
        ...
        // We'll remember the fact so that any blocking call is interrupted and
        // any
        // further attempt to use the socket will return ETERM. The user is
        // still
    ...
    @Override
    protected void process_bind(final Pipe pipe_)
    {
        attach_pipe(pipe_);
    }
    ...
    // method.
    protected boolean xsetsockopt(final int option_, final Object optval_)
    {
        return false;
    }
    ...
    protected void xhiccuped(final Pipe pipe_)
    {
        throw new UnsupportedOperationException("Must override");
    }
    ...
    public void in_event()
    {
        ...
        catch (final ZError.CtxTerminatedException e) {
        }
    ...
    {
        // If the object was already marked as destroyed, finish the
        // deallocation.
        if (destroyed) {
    ...
    @Override
    public void read_activated(final Pipe pipe_)
    {
        xread_activated(pipe_);
    }

    @Override
    public void write_activated(final Pipe pipe_)
    {
        xwrite_activated(pipe_);
    }

    @Override
    public void hiccuped(final Pipe pipe_)
    {
    ...
    public void terminated(final Pipe pipe_)
    {
        ...
        final Iterator<Entry<String, Pipe>> it = inprocs.entrySet().iterator();
        while (it.hasNext()) {
    ...
    private void extract_flags(final Msg msg_)
    {
    ...
    public boolean monitor(final String addr_, final int events_)
    {
        boolean rc;
        if (ctx_terminated) {
            throw new ZError.CtxTerminatedException();
    ...
    public void event_connected(final String addr, final SelectableChannel ch)
    {
    ...
    public void event_connect_delayed(final String addr, final int errno)
    {
    ...
    public void event_connect_retried(final String addr, final int interval)
    {
        if ((monitor_events & ZMQ.ZMQ_EVENT_CONNECT_RETRIED) == 0) {
            return;
        }

        monitor_event(new ZMQ.Event(ZMQ.ZMQ_EVENT_CONNECT_RETRIED, addr,
                                    interval));
    }

    public void event_listening(final String addr, final SelectableChannel ch)
    {
    ...
    public void event_bind_failed(final String addr, final int errno)
    {
    ...
    public void event_accepted(final String addr, final SelectableChannel ch)
    {
    ...
    public void event_accept_failed(final String addr, final int errno)
    {
    ...
    public void event_closed(final String addr, final SelectableChannel ch)
    {
    ...
    public void event_close_failed(final String addr, final int errno)
    {
    ...
    public void event_disconnected(final String addr, final SelectableChannel ch)
    {
    ...
    protected void monitor_event(final ZMQ.Event event)
    {
    ...
    protected void stop_monitor()
    {
        ...
        if (monitor_socket != null) {
            if ((monitor_events & ZMQ.ZMQ_EVENT_MONITOR_STOPPED) != 0) {
                monitor_event(new ZMQ.Event(ZMQ.ZMQ_EVENT_MONITOR_STOPPED, "",
                                            0));
            }
