...
public class Decoder extends DecoderBase
{
    ...
    public Decoder(final int bufsize_, final long maxmsgsize_)
    {
        super(bufsize_);
    ...
    {
        // First byte of size is read. If it is 0xff(-1 for java byte) read
        // 8-byte size.
        // Otherwise allocate the buffer for message data and read the
        // message data into it.
        final byte first = tmpbuf[0];
        if (first == -1) {
    ...
    private boolean flags_ready()
    {
        ...
        final int first = tmpbuf[0];

        in_progress.setFlags(first & Msg.MORE);

        next_step(in_progress, message_ready);
    ...
    private boolean message_ready()
    {
        ...
        final int rc = msg_sink.push_msg(in_progress);
        if (rc != 0) {
            return false;
        }
