...
abstract public class DecoderBase implements IDecoder
{
    ...
    public DecoderBase(final int bufsize_)
    {
        state = -1;
        to_read = 0;
        bufsize = bufsize_;
    ...
    @Override
    public int process_buffer(final ByteBuffer buf_, final int size_)
    {
        ...
        while (true) {
            ...
            final int to_copy = Math.min(to_read, size_ - pos);
    ...
    protected void next_step(final byte[] buf_, final int to_read_,
                             final int state_)
    {
        read_buf = buf_;
        read_pos = 0;
