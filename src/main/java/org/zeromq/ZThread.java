...
public class ZThread
{
    ...
    private static class ShimThread extends Thread
    {
        ...
        protected ShimThread(final ZContext ctx,
                             final IAttachedRunnable runnable,
                             final Object[] args, final Socket pipe)
        {
            assert (ctx != null);
            assert (pipe != null);
        ...
        public ShimThread(final IDetachedRunnable runnable, final Object[] args)
        {
            assert (runnable != null);
    ...
    public static void start(final IDetachedRunnable runnable,
                             final Object... args)
    {
        // Prepare child thread
        final Thread shim = new ShimThread(runnable, args);
        shim.start();
    }
    ...
    public static Socket fork(final ZContext ctx,
                              final IAttachedRunnable runnable,
                              final Object... args)
    {
        final Socket pipe = ctx.createSocket(ZMQ.PAIR);
        ...
        final ZContext ccontext = ZContext.shadow(ctx);
        final Socket cpipe = ccontext.createSocket(ZMQ.PAIR);
        if (cpipe == null) {
            return null;
        }
        ...
        // Prepare child thread
        final Thread shim = new ShimThread(ccontext, runnable, args, cpipe);
        shim.start();
