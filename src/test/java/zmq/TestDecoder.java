...
public class TestDecoder
{
    ...
    private int read_short_message(final ByteBuffer buf)
    {
        buf.put((byte) 6);
        buf.put((byte) 0); // flag
    ...
    // as if it read data from socket
    private int read_long_message1(final ByteBuffer buf)
    {
    ...
    private int read_long_message2(final ByteBuffer buf)
    {
    ...
    {
        final ByteBuffer in = decoder.get_buffer();
        final int insize = read_short_message(in);

        assertThat(insize, is(7));
        in.flip();
        final int process = decoder.process_buffer(in, insize);
    ...
    public void testReaderMultipleMsg()
    {
        ...
        in.flip();
        final int processed = decoder.process_buffer(in, 14);
    ...
    static class CustomDecoder extends DecoderBase
    {
        ...
        public CustomDecoder(final int bufsize_, final long maxmsgsize_)
        {
            super(bufsize_);
        ...
        private boolean read_header()
        {
            ...
            final ByteBuffer b = ByteBuffer.wrap(header, 6, 4);
            size = b.getInt();
        ...
        @Override
        public void set_msg_sink(final IMsgSink msg_sink)
        {
            sink = msg_sink;
        }
    ...
    {
        final CustomDecoder cdecoder = new CustomDecoder(32, 64);
        cdecoder.set_msg_sink(session);

        final ByteBuffer in = cdecoder.get_buffer();
        final int insize = read_header(in);
        assertThat(insize, is(10));
        read_body(in);

        in.flip();
        final int processed = cdecoder.process_buffer(in, 30);
        assertThat(processed, is(30));
        assertThat(cdecoder.size, is(20));
        assertThat(session.out.size(), is(1));
    }

    private void read_body(final ByteBuffer in)
    {
        in.put("1234567890".getBytes(ZMQ.CHARSET));
        in.put("1234567890".getBytes(ZMQ.CHARSET));
    }

    private int read_header(final ByteBuffer in)
    {
