...
public class TestConnectDelay
{
    ...
    public void testConnectDelay1() throws Exception
    {
        ...
        final SocketBase to = ZMQ.zmq_socket(context, ZMQ.ZMQ_PULL);
        assert (to != null);
        ...
        // Create a socket pushing to two endpoints - only 1 message should
        // arrive.
        final SocketBase from = ZMQ.zmq_socket(context, ZMQ.ZMQ_PUSH);
        assert (from != null);
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            message += ('0' + i);
            final int sent = ZMQ.zmq_send(from, message, 0);
            assert (sent >= 0);
        }
        ...
        int seen = 0;
        for (int i = 0; i < 10; ++i) {
            final Msg msg = ZMQ.zmq_recv(to, 0);
            if (msg == null) {
                break;
            }
            seen++;
    ...
    public void testConnectDelay2() throws Exception
    {
        ...
        // Create a socket pushing to two endpoints - all messages should
        // arrive.
        final SocketBase from = ZMQ.zmq_socket(context, ZMQ.ZMQ_PUSH);
        assert (from != null);
    ...
    public void testConnectDelay3() throws Exception
    {
        ...
        final SocketBase frontend = ZMQ.zmq_socket(context, ZMQ.ZMQ_DEALER);
