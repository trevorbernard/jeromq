...
public class TestMonitor
{
    ...
    static class SocketMonitor extends Thread
    {
        ...
        public SocketMonitor(final Ctx ctx, final String monitor_addr)
        {
            this.ctx = ctx;
        ...
        public void run()
        {
            ...
            final SocketBase s = ZMQ.zmq_socket(ctx, ZMQ.ZMQ_PAIR);
            final boolean rc = s.connect(monitor_addr);
            assert (rc);
            // Only some of the exceptional events could fire
            while (true) {
                final ZMQ.Event event = ZMQ.Event.read(s);
    ...
    {
        final String addr = "tcp://127.0.0.1:5590";
        final SocketMonitor[] threads = new SocketMonitor[3];
        // Create the infrastructure
        final Ctx ctx = ZMQ.zmq_init(1);
        assert (ctx != null);
        // set socket monitor
        final SocketBase rep = ZMQ.zmq_socket(ctx, ZMQ.ZMQ_REP);
        assert (rep != null);
        try {
        ...
        boolean rc = ZMQ.zmq_socket_monitor(rep, "inproc://monitor.rep",
                                            ZMQ.ZMQ_EVENT_ALL);
        ...
        final SocketBase req = ZMQ.zmq_socket(ctx, ZMQ.ZMQ_REQ);
        assert (req != null);

        // REQ socket monitor, all events
        rc = ZMQ.zmq_socket_monitor(req, "inproc://monitor.req",
                                    ZMQ.ZMQ_EVENT_ALL);
        assertEquals(true, rc);
        ...
        // 2nd REQ socket
        final SocketBase req2 = ZMQ.zmq_socket(ctx, ZMQ.ZMQ_REQ);
        assert (req2 != null);

        // 2nd REQ socket monitor, connected event only
        rc = ZMQ.zmq_socket_monitor(req2, "inproc://monitor.req2",
                                    ZMQ.ZMQ_EVENT_CONNECTED);
        assertEquals(true, rc);
