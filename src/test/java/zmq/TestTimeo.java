...
public class TestTimeo
{
    class Worker implements Runnable
    {
        ...
        {
            ZMQ.zmq_sleep(1);
            final SocketBase sc = ZMQ.zmq_socket(ctx, ZMQ.ZMQ_PUSH);
            assertThat(sc, notNullValue());
            final boolean rc = ZMQ.zmq_connect(sc, "inproc://timeout_test");
    ...
    public void testTimeo() throws Exception
    {
        ...
        final SocketBase sb = ZMQ.zmq_socket(ctx, ZMQ.ZMQ_PULL);
        ...
        timeout = 2000;
        ZMQ.zmq_setsockopt(sb, ZMQ.ZMQ_RCVTIMEO, timeout);
        final Thread thread = new Thread(new Worker(ctx));
        thread.start();
        ...
        assertThat(rc, is(true));
        final Msg smsg = new Msg(
                                 "12345678ABCDEFGH12345678abcdefgh".getBytes(ZMQ.CHARSET));
        final int r = ZMQ.zmq_send(sc, smsg, 0);
        assertThat(r, is(32));
        msg = ZMQ.zmq_recv(sb, 0);
