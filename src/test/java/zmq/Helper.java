...
public class Helper
{
    ...
    public static class DummySocketChannel implements WritableByteChannel
    {
        ...
        public DummySocketChannel(final int bufsize)
        {
    ...
    public static class DummySession extends SessionBase
    {
        ...
        public DummySession()
        {
            this(new DummyIOThread(), false, new DummySocket(), new Options(),
                 new Address("tcp", "localhost:9090", false));
        }

        public DummySession(final IOThread io_thread_, final boolean connect_,
        ...
        @Override
        public int push_msg(final Msg msg)
        {
    ...
    public static void bounce(final SocketBase sb, final SocketBase sc)
    {
    ...
    public static void send(final Socket sa, final String data)
                                                               throws IOException
    {
        final byte[] content = data.getBytes(ZMQ.CHARSET);

        final byte[] length = String.format("%04d", content.length)
        ...
        int rc;
        // Bounce the message back.
        final InputStream in = sa.getInputStream();
        final OutputStream out = sa.getOutputStream();
        ...
        while (to_read > 0) {
            ...
            read += rc;
            to_read -= rc;
            System.out.println("read " + rc + " total_read " + read
                               + " to_read " + to_read);
        }
        System.out.println(String.format("%02x %02x %02x %02x", buf[0], buf[1],
                                         buf[2], buf[3]));
        try {
            Thread.sleep(1000);
        }
        ...
        System.out.println("recv " + reslen + " "
                           + new String(buf, 0, reslen, ZMQ.CHARSET));
