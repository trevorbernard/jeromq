...
public class peering3
{
    ...
    private static final String WORKER_READY = "\001"; // Signals worker is
                                                       // ready
    ...
    private static class client_task extends Thread
    {
        ...
        public void run()
        {
            ...
            while (true) {
                ...
                while (burst > 0) {
                    final String taskId = String.format("%04X",
                                                        rand.nextInt(10000));
                    // Send request, get reply
                    client.send(taskId, 0);

                    // Wait max ten seconds for a reply, then complain
                    final PollItem pollSet[] = { new PollItem(client,
                                                              Poller.POLLIN) };
                    ...
                    if (pollSet[0].isReadable()) {
                        ...
                        monitor.send(String.format("E: CLIENT EXIT - lost task %s",
                                                   taskId), 0);
                        ctx.destroy();
                        return;
                    }
    ...
    public static void main(final String[] argv)
    {
        ...
        while (true) {
            ...
            // for. We may reroute requests from our local frontend, but not
            // from //
            ...
            while (localCapacity + cloudCapacity > 0) {
                final PollItem secondary[] = {
                                              new PollItem(localfe,
                                                           Poller.POLLIN),
                                              new PollItem(cloudfe,
                                                           Poller.POLLIN) };
