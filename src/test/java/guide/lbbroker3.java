...
public class lbbroker3
{
     ...
     * reactor passes it to a handler function. We have two handlers; one for
     * the frontend, one for the backend:
     */
                          ...
                          final Object arg_)
        {
            ...
            if (msg != null) {
                ...
                if (arg.workers.size() == 1) {
                    final PollItem newItem = new PollItem(arg.frontend,
                                                          ZMQ.Poller.POLLIN);
    ...
    /**
     * And the main task now sets-up child tasks, then starts its reactor. If
     * you press Ctrl-C, the reactor exits and the main task shuts down.
     */
